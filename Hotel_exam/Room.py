...
# Clase Room
class Room():
    room_count = 0 # Para asegurarse de que no se repiten el numero de habitaciones para cada hotel
    def __init__(self, room_type, room_number, room_state, room_price):
        self.room_type = room_type
        Room.room_count += 1  # Incrementa el contador de habitaciones
        self.room_number = Room.room_count # Habitación Única
    ...
    def get_room_type(self):
        if isinstance(self.room_type, HotelRoomType):
            ...
            raise TypeError("La habitación debe pertenecer al tipo de habitaciones del hotel")
        
    def get_room_number(self):
        if isinstance(Room.room_count, int):
            return Room.room_count
        else:
            raise TypeError("La habitacion debe ser un número entero")
        
    def get_room_state(self):
        if isinstance(self.room_state, bool):
            return self.room_state
        else:
            raise TypeError("El estado del habitacion debe ser un booleano.")
        
    def get_room_price(self):
        if isinstance(self.room_price, float):
            return self.room_price
        else:
            raise TypeError("La habitacion debe tener un precio con numeros decimales.")
    # El estado cambia en funcion de si esta limpio o no     
    def set_room_state(self, room_state):
        self.room_state = room_state
    # El precio cabia dependiendo de la temporada, descuentos, etc
    ...
    # Métodos para marcar la habitación como ocupada (check-in) o desocupada (check-out)
    def check_in(self):
